export const PREDICCION_LIMITS = {
...
export const PREDICCION_FEATURE_NAMES = [
  "Edad",
  "Diag.Ing1",
  "Diag.Ing2",
  "Diag.Egr2",
...
export interface ExplicacionRequest extends PrediccionRequest {
...
export async function runPrediccion(
  ...
  try {
    ...
    return response.json() as Promise<PrediccionResponse>;
  } catch (err: unknown) {
...
export async function runExplicacion(
  ...
  try {
    const response = await fetch("/api/prediccion/explain", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(data),
      signal: controller.signal,
