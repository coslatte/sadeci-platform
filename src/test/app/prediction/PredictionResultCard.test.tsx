...
import { PredictionResultCard } from "@/app/prediction/components/PredictionResultCard";
import {
...
describe("PredictionResultCard", () => {
  ...
  it("shows survives label at exactly 0", () => {
    const { container } = render(<PredictionResultCard probability={0} />);
    expect(
      within(container).getByText(PREDICCION_PATIENT_SURVIVES),
    ).toBeTruthy();
  });
  ...
  it("shows dies label at exactly 0.5", () => {
    const { container } = render(<PredictionResultCard probability={0.5} />);
    expect(within(container).getByText(PREDICCION_PATIENT_DIES)).toBeTruthy();
  });

  it("displays the probability formatted to 1 decimal percentage", () => {
    const { container } = render(<PredictionResultCard probability={0.723} />);
  ...
  it("displays 100.0% for probability 1", () => {
    const { container } = render(<PredictionResultCard probability={1} />);
